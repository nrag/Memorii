...
def _execution_output() -> dict[str, object]:
    scenario = _scenario_by_id("evolution_abandoned_then_resumed_work")
    return expected_memory_evolution_decision_for_checkpoint(
        scenario=scenario,
        checkpoint=scenario.checkpoints[0],
    ).model_dump(mode="json")
...
@pytest.mark.parametrize(
    "placement",
    ["suppressed_only", "rejected", "context", "historical", "mixed"],
)
def test_memory_evolution_suppressed_branch_exclusion_truth_table_accepts_or_channels(
    placement: str,
) -> None:
    output = _execution_output()
    execution = output["execution_selection"]
    retrieval = output["retrieval_context"]
    assert isinstance(execution, dict)
    assert isinstance(retrieval, dict)
    suppressed = list(execution["suppressed_branch_memory_ids"])
    ...
    for key in (
        ...
        retrieval[key] = [
            memory_id
            for memory_id in retrieval[key]
            if memory_id not in suppressed
        ]
    if placement in {"rejected", "context", "historical"}:
        retrieval[f"{'query_' if placement != 'rejected' else ''}{placement}_memory_ids"] = suppressed
    elif placement == "mixed":
        retrieval["rejected_memory_ids"] = [suppressed[0]]
        retrieval["query_context_memory_ids"] = [suppressed[1]]
...
@pytest.mark.parametrize(
    ("section", "field"),
    [
        ("answer_selection", "selected_memory_ids"),
        ("answer_selection", "supporting_memory_ids"),
        ("answer_selection", "citation_memory_ids"),
        ("lifecycle_snapshot", "checkpoint_active_record_ids"),
    ],
)
def test_memory_evolution_suppressed_branch_truth_table_rejects_direct_or_active_placement(
    section: str,
    field: str,
) -> None:
    output = _execution_output()
    ...
    suppressed_id = execution["suppressed_branch_memory_ids"][0]
    target[field] = [*target[field], suppressed_id]
