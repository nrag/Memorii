...
def test_memory_evolution_sim_long_horizon_execution_has_action_state_pressure() -> None:
    ...
    assert any(claim.claim_id.endswith("branch_b_progress") for claim in scenario.claims if claim.claim_id in checkpoint.expected_claim_ids)
    assert any(claim_id.endswith("branch_a_blocked") for claim_id in checkpoint.expected_excluded_claim_ids)
    assert any(observation.event_id.endswith("branch_b_progress") and observation.phase == "evolution" for observation in scenario.observations)
