...
def align_runtime_graph_to_oracle(
    *, scenario: LatentGraphScenario, graph_items: list[RuntimeGraphItem]
    ...
    runtime_entity_by_canonical_id = {item.canonical_id: item for item in runtime_entities if item.canonical_id}
    oracle_entity_by_id = {
        entity.entity_id: entity for entity in scenario.entities if entity.observability != ObservabilityLabel.HIDDEN
    }
    for runtime in runtime_entities:
        ...
        alignments.append(
            ...
            or RuntimeGraphAlignment(
                ...
                score=0.0,
                rationale="no oracle entity candidates",
            )
        )
    for runtime in runtime_claims:
        direct = next(
            (
                ...
                if claim.claim_id == runtime.claim_id and claim.observability != ObservabilityLabel.HIDDEN
            ),
            None,
        )
        if direct is not None:
            ...
            alignments.append(
                RuntimeGraphAlignment(
                    ...
                    if has_provenance
                    else RuntimeGraphAlignmentVerdict.PARTIAL,
                    score=1.0 if has_provenance else 0.8,
                    matched_on=["claim_id", "evidence_event_ids"] if has_provenance else ["claim_id"],
                    rationale="runtime claim id and provenance match latent claim"
                    if has_provenance
                    else "claim id matches but provenance is missing",
                )
            )
            continue
        best = _best_alignment(
    ...
    for entity in scenario.entities:
        if entity.observability != ObservabilityLabel.HIDDEN and not any(
            a.oracle_item_id == entity.entity_id and a.item_type == "entity" for a in alignments
        ):
            alignments.append(
                RuntimeGraphAlignment(
                    oracle_item_id=entity.entity_id,
                    ...
                    score=0.0,
                    rationale="oracle entity missing from runtime graph",
                )
            )
    for claim in scenario.claims:
        if claim.observability != ObservabilityLabel.HIDDEN and not any(
            a.oracle_item_id == claim.claim_id and a.item_type == "claim" for a in alignments
        ):
            alignments.append(
                RuntimeGraphAlignment(
                    oracle_item_id=claim.claim_id,
                    item_type="claim",
                    verdict=RuntimeGraphAlignmentVerdict.MISSING_EXPECTED,
                    score=0.0,
...
def _align_claim_with_entity_context(
    *,
    runtime: RuntimeClaimGraphItemRow,
    oracle_claim: LatentClaim,
    runtime_entity_by_canonical_id: Mapping[str, RuntimeEntityGraphItemRow],
...
def _runtime_claim_entity_matches(
    *,
    ...
    runtime_name: str,
    runtime_entities_by_canonical_id: Mapping[str, RuntimeEntityGraphItemRow],
    oracle_entity: LatentEntity,
) -> bool:
    runtime_entity = runtime_entities_by_canonical_id.get(runtime_entity_id)
    runtime_names = _runtime_entity_names(
        runtime_entity=runtime_entity, fallback_name=runtime_name, fallback_entity_id=runtime_entity_id
    )
    ...
    if oracle_type and runtime_type not in {"", "unknown", oracle_type}:
        return False
    return _runtime_names_are_safe_alias(
        runtime_names=runtime_names, oracle_names=oracle_names, oracle_type=oracle_type
    )


def _runtime_entity_names(
    *, runtime_entity: RuntimeEntityGraphItemRow | None, fallback_name: str, fallback_entity_id: str
) -> set[str]:
    names = {
...
def _best_alignment(alignments: list[RuntimeGraphAlignment]) -> RuntimeGraphAlignment | None:
    if not alignments:
        return None
    alignments = sorted(
        alignments, key=lambda item: (item.score, item.verdict == RuntimeGraphAlignmentVerdict.ALIGNED), reverse=True
    )
    best = alignments[0]
    if (
        len(alignments) > 1
        and best.score == alignments[1].score
        and best.score > 0.0
        and best.oracle_item_id != alignments[1].oracle_item_id
    ):
        return best.model_copy(
            update={
                "verdict": RuntimeGraphAlignmentVerdict.AMBIGUOUS_ALIGNMENT,
                "rationale": f"ambiguous alignment between {best.oracle_item_id} and {alignments[1].oracle_item_id}",
            }
        )
    return best
...
def _enforce_one_to_one_alignment(alignments: list[RuntimeGraphAlignment]) -> list[RuntimeGraphAlignment]:
    ...
    for alignment in alignments:
        ...
        if current is None or (alignment.score, str(alignment.runtime_item_id)) > (
            current.score,
            str(current.runtime_item_id),
        ):
...
def _runtime_entity_fields(item: RuntimeEntityGraphItemRow) -> dict[str, object]:
    return {
