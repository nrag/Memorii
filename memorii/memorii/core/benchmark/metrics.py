...
def compute_metrics(observation: ScenarioObservation) -> ScenarioMetrics:
    ...
    if observation.relevant_ids:
        ...
        if precision is None:
            precision = 0.0
    elif observation.excluded_ids:
        excluded_hits = len(set(observation.retrieved_ids) & set(observation.excluded_ids))
        recall = 1.0 if excluded_hits == 0 else 0.0
        precision = 1.0 if not observation.retrieved_ids else 0.0
    ...
    return ScenarioMetrics(
        ...
        lifecycle_success_rate=_bool_metric(observation.lifecycle_success),
        active_memory_accuracy=_bool_metric(observation.active_memory_correct),
        inactive_memory_accuracy=_bool_metric(observation.inactive_memory_correct),
        archived_memory_accuracy=_bool_metric(observation.archived_memory_correct),
        retrieval_currentness_accuracy=_bool_metric(observation.retrieval_currentness_correct),
        duplicate_avoidance_accuracy=_bool_metric(observation.duplicate_avoidance_correct),
        scope_preservation_accuracy=_bool_metric(observation.scope_preservation_correct),
        pollution_avoidance_accuracy=_bool_metric(observation.pollution_avoidance_correct),
