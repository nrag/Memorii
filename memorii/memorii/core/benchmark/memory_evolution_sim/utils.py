...
def role_relation_ids(output: SimSystemOutput) -> list[str]:
...
def role_entity_ids(output: SimSystemOutput) -> list[str]:
    return ordered_unique([
...
def role_claim_ids(output: SimSystemOutput) -> list[str]:
    return ordered_unique([
...
def role_citation_event_ids(output: SimSystemOutput) -> list[str]:
    return ordered_unique([
...
def required_definition_claim_ids_for_selected_claims(
...
def bad_supporting_event_ids(
    ...
    for event_id in event_ids:
        if event_id in checkpoint.expected_citation_event_ids:
            continue
        observation = observation_by_id(scenario, event_id)
        if observation is not None and observation.modality in bad_modalities:
            bad.append(event_id)
    return bad

def context_only_noise_event_ids(scenario: LatentGraphScenario, output: SimSystemOutput) -> list[str]:
    bad: list[str] = []
...
def hidden_answer_leaks(scenario: LatentGraphScenario, output: SimSystemOutput) -> list[str]:
    text = normalize_sim_text(" ".join(item for item in [output.answer, output.next_action, output.rationale] if item))
    if not text:
        return []
    leaks: list[str] = []
    for entity in scenario.entities:
        ...
        if any(normalize_sim_text(name) and normalize_sim_text(name) in text for name in names):
            leaks.append(entity.entity_id)
    for claim in scenario.claims:
        if claim.observability != ObservabilityLabel.HIDDEN:
            continue
        if normalize_sim_text(claim.object.value) and normalize_sim_text(claim.object.value) in text:
            leaks.append(claim.claim_id)
    return ordered_unique(leaks)

def relation_bucket(checkpoint: OracleCheckpoint) -> str:
...
def claim_bucket(checkpoint: OracleCheckpoint) -> str:
    return {
...
def answer_bucket(checkpoint: OracleCheckpoint) -> str:
    return "abandoned_branch_selected" if checkpoint.expected_next_action else claim_bucket(checkpoint)

def normalize_sim_text(value: str | None) -> str:
    return " ".join(re.sub(r"[^a-z0-9]+", " ", (value or "").lower()).split())

def extract_rule_answer(text: str) -> str:
