...
class MemoryExtractionRunError(RuntimeError):
    ...
    def __init__(self, run: ExtractionRun) -> None:
        if (
            run.status not in {ExtractionRunStatus.FAILED, ExtractionRunStatus.PARTIAL}
            or run.failure_code is None
        ):
            raise ValueError(
                "non-committable extraction requires a failed or partial run with a failure code"
            )
        super().__init__(
            f"memory extraction is not commit-eligible: "
            f"{run.status.value}:{run.failure_code.value}"
        )
        self.run = run
