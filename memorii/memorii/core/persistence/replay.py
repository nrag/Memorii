...
class ReplayService:
    ...
    def replay_task_events(self, task_id: str) -> tuple[dict[str, ExecutionNode], dict[str, ExecutionEdge]]:
        ...
        for event in self._ordered_events(self._event_log_store.list_by_task(task_id)):
    ...
    def replay_solver_events(self, solver_run_id: str) -> tuple[dict[str, SolverNode], dict[str, SolverEdge]]:
        ...
        for event in self._ordered_events(self._event_log_store.list_by_solver_run(solver_run_id)):
            self._apply_solver_event(event=event, nodes=nodes, edges=edges)
        return nodes, edges

    def _ordered_events(self, events: list[EventRecord]) -> list[EventRecord]:
    ...
    def _apply_execution_event(
        ...
        if event.event_type in {EventType.NODE_ADDED, EventType.NODE_COMMITTED, EventType.STATUS_UPDATED}:
            node = ExecutionNode.model_validate(entity)
            nodes[node.id] = node
        elif event.event_type in {EventType.EDGE_ADDED, EventType.EDGE_COMMITTED}:
    ...
    def _apply_solver_event(
        ...
        if event.event_type in {EventType.NODE_ADDED, EventType.NODE_COMMITTED, EventType.NODE_REOPENED, EventType.STATUS_UPDATED}:
