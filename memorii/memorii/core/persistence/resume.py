...
class ResumeService:
    ...
    def load_solver_graph(self, solver_run_id: str) -> SolverResumeState:
        ...
        latest_overlay = self._latest_committed_overlay(solver_run_id)
        ...
        if latest_overlay is not None:
            for node_overlay in latest_overlay.node_overlays:
                ...
                if node_overlay.reopenable or node_overlay.status == SolverNodeStatus.REOPENABLE:
        ...
        if latest_overlay is not None:
            overlay_by_node = {
                node_overlay.node_id: node_overlay for node_overlay in latest_overlay.node_overlays
            }

        for node in nodes:
            overlay = overlay_by_node.get(node.id)
            if node.type == SolverNodeType.QUESTION:
                if overlay is None or overlay.status not in {SolverNodeStatus.RESOLVED, SolverNodeStatus.ARCHIVED, SolverNodeStatus.MERGED}:
                    unresolved_questions.append(node.id)

            if node.type == SolverNodeType.OBSERVATION and overlay is not None and overlay.unexplained:
                unexplained_observations.append(node.id)
    ...
    def _latest_committed_overlay(self, solver_run_id: str) -> SolverOverlayVersion | None:
        versions = self._overlay_store.list_versions(solver_run_id)
