...
class InMemoryEventLogStore(EventLogStore):
    ...
    def list_by_task(self, task_id: str) -> list[EventRecord]:
        return sorted(self._by_task.get(task_id, []), key=lambda event: (event.timestamp, event.event_id))

    def list_by_solver_run(self, solver_run_id: str) -> list[EventRecord]:
        return sorted(self._by_solver.get(solver_run_id, []), key=lambda event: (event.timestamp, event.event_id))
