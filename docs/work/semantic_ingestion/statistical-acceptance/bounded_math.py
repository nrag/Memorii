...
class ArithmeticMeter:
    ...
    def reserve(self, numerator_bits: int, denominator_bits: int = 1) -> None:
        if (type(numerator_bits) is not int or type(denominator_bits) is not int
    ...
    def add(self, left: Fraction, right: Fraction) -> Fraction:
        ...
        self.reserve(max(_bits(left.numerator) + _bits(right.denominator),
                         _bits(right.numerator) + _bits(left.denominator)) + 1,
    ...
    def mul(self, left: Fraction, right: Fraction) -> Fraction:
        self.check(left)
        self.check(right)
        self.reserve(_bits(left.numerator) + _bits(right.numerator),
                     _bits(left.denominator) + _bits(right.denominator))
    ...
    def div(self, left: Fraction, right: Fraction) -> Fraction:
        ...
        self.reserve(_bits(left.numerator) + _bits(right.denominator),
                     _bits(left.denominator) + _bits(right.numerator))
...
def parse_rational(numerator: str, denominator: str, meter: ArithmeticMeter) -> Fraction:
    ...
    if (not magnitude or not magnitude.isascii() or not magnitude.isdigit()
            or (len(magnitude) > 1 and magnitude[0] == "0") or numerator == "-0"
            or not denominator or not denominator.isascii() or not denominator.isdigit()
            or denominator[0] == "0"):
        raise ValueError("malformed_rational")
    ...
    if projected > bits:
        ...
        limit = str(maximum)
        if any(len(s) > len(limit) or (len(s) == len(limit) and s > limit)
               for s in (magnitude, denominator)):
            raise ValueError("resource_limit")
...
def exact_binomial_tail(trials: int, successes: int, null_probability: Fraction, tail: Tail, meter: ArithmeticMeter) -> Fraction:
    if (type(trials) is not int or type(successes) is not int or trials < 1
            or not 0 <= successes <= trials or tail not in {"greater_or_equal", "less_or_equal"}
            or meter.compare(null_probability, _ZERO) < 0 or meter.compare(null_probability, _ONE) > 0):
        raise ValueError("out_of_domain")
...
def clopper_pearson_bound(trials: int, successes: int, alpha: Fraction, direction: Direction, steps: int, meter: ArithmeticMeter) -> RationalInterval:
    _alpha(alpha, meter)
    if (type(steps) is not int or steps < 1 or type(trials) is not int or trials < 1
            or type(successes) is not int or not 0 <= successes <= trials or direction not in {"lower", "upper"}):
        raise ValueError("out_of_domain")
...
def _weighted(observations: tuple[WeightedObservation, ...], meter: ArithmeticMeter) -> tuple[Fraction, Fraction, Fraction, Fraction]:
    ...
    for item in observations:
        if (type(item) is not WeightedObservation or meter.compare(item.weight, _ZERO) < 0
                or meter.compare(item.lower, item.upper) > 0
                or meter.compare(item.value, item.lower) < 0 or meter.compare(item.value, item.upper) > 0):
            raise ValueError("invalid_weight_or_range")
...
def _probability(mean: Fraction, squared: Fraction, threshold: Fraction, direction: Direction, terms: int, meter: ArithmeticMeter) -> HoeffdingResult:
...
def weighted_hoeffding(observations: tuple[WeightedObservation, ...], threshold: Fraction, direction: Direction, terms: int, meter: ArithmeticMeter) -> HoeffdingResult:
    mean, squared, _, _ = _weighted(observations, meter)
    return _probability(mean, squared, threshold, direction, terms, meter)


def invert_weighted_hoeffding(observations: tuple[WeightedObservation, ...], threshold: Fraction, direction: Direction, alpha: Fraction, steps: int, terms: int, meter: ArithmeticMeter) -> RationalInterval:
    _alpha(alpha, meter)
...
def holm_complete(claims: tuple[HolmClaim, ...], expected_locators: tuple[bytes, ...], family_alpha: Fraction, meter: ArithmeticMeter) -> tuple[HolmDecision, ...]:
    _alpha(family_alpha, meter)
    if (not claims or any(type(c) is not HolmClaim or type(c.locator_bytes) is not bytes or not c.locator_bytes for c in claims)
    ...
    def compare(left: HolmClaim, right: HolmClaim) -> int:
        if left.exact_order is not None and right.exact_order is not None:
            ...
            return 1
        elif (left.probability.lower == left.probability.upper
    ...
    try:
        ...
        return tuple(HolmDecision(c.locator_bytes, c.nominal_alpha if meter.compare(c.nominal_alpha, conservative) < 0 else conservative, "inconclusive", None) for c in sorted(claims, key=lambda c: c.locator_bytes))
    blocked = False
