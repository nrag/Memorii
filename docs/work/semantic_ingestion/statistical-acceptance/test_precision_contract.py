...
from bounded_math import ArithmeticBudget, ArithmeticMeter, clopper_pearson_bound
from test_wire_contract import inputs, raw, synthetic_binding
from wire_contract import EnclosedValue, evaluate_certificate, verify_certificate
...
def test_public_bounds_honor_precision(method, direction, labels, coarse, fine):
    ...
    policy['memberships'] = [
        {**member, 'cluster_id': f'cluster_{i}', 'provenance_ids': [f'source_{i}'],
         'expected_event_ids': [f'event_{i}'],
         'weight': {'encoding_spec_id': 'weight', 'fixed_scale_value': '0.50'}}
        for i in range(2)
    ]
    evidence['events'] = [
        {**event, 'event_id': f'event_{i}', 'provenance_id': f'source_{i}',
    ...
    for precision, expected in ((6, coarse), (8, fine)):
        ...
        actual = (Fraction(bound.lower.numerator, bound.lower.denominator),
...
def test_alpha_one_retains_exact_cp_endpoint(successes, direction, point):
    result = clopper_pearson_bound(3, successes, Fraction(1), direction, 8,
                                 ArithmeticMeter(ArithmeticBudget(1024, 40000)))
