...
def synthetic_binding(
    ...
    parsed = wire_contract._policy(policy, limits)
...
def inputs(
    ...
    limits = TransportLimits(
        40000, 40000, 40000, 2048, 50000, 12, 40000, 8, 32, 8000, 2000
    )
...
def mutated(candidate_bytes: bytes, change) -> bytes:
    body = decode_typed_value(candidate_bytes)
...
def test_known_upper_tail_and_cp_endpoint_values() -> None:
    ...
    result = verify_certificate(
        BytesIO(candidate(p, e, b, limits)), BytesIO(p), BytesIO(e), b, limits
    ).results[0]
...
def test_reencoded_candidate_fields_cannot_override_recomputation(field: str) -> None:
    ...
    def change(body: dict[str, object]) -> None:
        ...
        body["results"] = rows
        rows[0][field] = (
            9 if field == "observed_successes" else {"numerator": 9, "denominator": 10}
        )
...
def test_candidate_shape_and_held_document_mutations_reject() -> None:
    ...
    for change in (
        ...
        with pytest.raises(WireRejected, match="certificate_mismatch"):
            verify_certificate(
                BytesIO(mutated(c, change)), BytesIO(p), BytesIO(e), b, limits
            )
    with pytest.raises(WireRejected, match="held_authority_digest"):
        verify_certificate(
            BytesIO(c), BytesIO(p.replace(b"0.50", b"0.49", 1)), BytesIO(e), b, limits
        )
    with pytest.raises(WireRejected, match="held_authority_digest"):
        verify_certificate(
            BytesIO(c), BytesIO(p), BytesIO(e.replace(b"0.00", b"1.00", 1)), b, limits
        )
...
def test_public_transport_exact_limits_and_limit_minus_one_reject() -> None:
    ...
    assert verify_certificate(
        BytesIO(certificate), BytesIO(p), BytesIO(e), binding, exact
    ).accepted
    ...
    for field, stream in (
        ...
        with pytest.raises(WireRejected, match="transport_limit"):
            verify_certificate(
                BytesIO(certificate), BytesIO(p), BytesIO(e), binding, reduced
            )
    ...
    (
        ...
        (
            ...
            lambda p, e: (
                p,
                e.replace(
                    b'"event_id":"event"', b'"event_id":"event","event_id":"other"', 1
                ),
            ),
        ),
    ),
...
def test_duplicate_json_keys_reject_before_context(target: str, changed) -> None:
    ...
    with pytest.raises(WireRejected, match="duplicate_json_key"):
        evaluate_certificate(
            BytesIO(changed_policy), BytesIO(changed_evidence), admitted, limits
        )
...
def test_decimal_grammar_precision_and_zero_alpha_reject(
    ...
    with pytest.raises(WireRejected, match=reason):
        evaluate_certificate(
            BytesIO(changed), BytesIO(e), synthetic_binding(changed, e, limits), limits
        )
...
def test_short_reads_integer_limit_and_output_preflight() -> None:
    ...
    calls: list[bool] = []
    wire_contract.ENCODER_HOOK = lambda: calls.append(True)
    try:
        ...
        wire_contract.ENCODER_HOOK = None
    assert not calls
...
def test_unicode_locator_and_exact_output_cap() -> None:
    ...
    encoded: list[bool] = []
    wire_contract.MATERIALIZER_HOOK = lambda: materialized.append(True)
    wire_contract.ENCODER_HOOK = lambda: encoded.append(True)
    try:
        exact = candidate(exact_policy, e2, exact_binding, limits)
    finally:
        wire_contract.MATERIALIZER_HOOK = None
        wire_contract.ENCODER_HOOK = None
...
@pytest.mark.parametrize(
    ...
    (
        ...
        lambda policy: policy["memberships"][0].__setitem__(
            "provenance_ids", ["other-source"]
        ),
        lambda policy: policy["memberships"][0].__setitem__(
            "expected_event_ids", ["other-event"]
        ),
...
@pytest.mark.parametrize(
    "field",
    tuple(field.name for field in wire_contract.fields(NumericAuthority)),
)
