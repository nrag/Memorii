# Semantic Ingestion Closure Resume

Work type: implementation. Coordinator: root. Status: active, not closed.
Base: e33d44cef6d815cda4bd78a5acd980344404c4ed on semantic_ingestion_m5.
Parent: implementation.plan.md. Current packet:
../observation-ledger/milestones/continuation-runtime.plan.md.
The previous resume is preserved in archive/resume-8ee55711246d60269478d8d7201335f1222d2c5408a95bdbc4b7d56aa8c4b9ac.md.

## Current Evidence And Remaining Work

The cursor design is approved and promoted. Registered graph and ingestion-time
paging, separate signature purposes and shared bounded retention are implemented.
Corrected component candidated19c7bfc93eebd730ff14f0125ee5887d0975cbc5a72008905f28c28244c0729
passes30 paging/contract and13 codec/cursor tests. Full Ruff and canonical scoped
Pyright pass. Full independent publication equality covers180 schemas/1262roles;
58 vectors pass. Portable logs and exact limits are in the component-evidence
folder beside the current packet. Independent bounded component delta reviews approve; this is not parent closure.
No current GitHub CI, installed package or full activation gate is claimed.
The activation run was interrupted after review findings, with no completed test.

Checkpoint6029030d (pushed) in atomic_store.py and graph_observation_cohort.py
provides detached authority reconstruction/membership only. It is not a complete
materializer or public provider integration and must not be reported as one.
Concrete bounded cohort construction, all17 observed streams, protected host
composition, checkpoint context, comparator and activated failure proofs remain.

The preserved acceptance successor remains rejected historical evidence. The
new ../acceptance-issuance-prefix/closure.md records an independently approved
correction with103 model proofs, committed pair lookup and atomic issue-time
head/status capture. This does not implement or promote production acceptance
artifacts; R14 is partial rather than design-blocked. Ingestion-time persistence has a separate unapproved draft;
source-admission anchoring, source-result preimages and acyclic batch binding
need an exact contract before production edits. Real keys do not resolve either
contract gap. Monitoring, final host release configuration, evaluator/CLI and
qualifying measurements remain incomplete.

All23 table:15 retained baseline-complete, seven partial and one not
implemented. No row closed this round. Target remains fewer than three open;
never lower the count by relabeling unmet criteria. Actual trusted production
key provisioning and final signatures remain deferred; engineering signing and
verification paths must still be proven.

## Ownership And Next Action

Root owns code, tests and long commands. The issuance-prefix agent has handed
off its directory; root completed the bounded issuance-prefix correction (103 local tests and three independent approvals). A
read-only admission map confirms the canonical bootstrap/non-bootstrap distinction
and missing immutable admission digest. The activated provider test now also
checks detached membership, scope refusal, missing graph closure, and snapshot
stability after a subsequent write; it passed in1472.96s. The CI selection check also passes. Final test review confirms a production-entrypoint gap: both new owners currently have only test callers. This evidence verifies internal preparation, not runtime integration.

Next action: verify and review the approved retained-context provenance recipes in ../observed-provenance-materialization/design.plan.md, then resume the concrete materializer.

## Latest Gate Outcome

All three hermetic CTV/lifecycle/structural source gates pass. Source tests:
276 passed initially and3 stale-pin/dependency cases passed corrected reruns.
The final full Ruff, identity and scoped Pyright checks pass. Full activation,
installed-package and GitHub checks are not claimed for this component revision.

## Active Design Dependency And Internal Preparation

The provider/public methods and capability/host wiring are parked in
../observed-provenance-materialization/public-composition-draft.patch. They are
not active production code: the concrete materializer remains absent. Current
internal preparation comprises the registered access adapter,17 observed emission
roots and4-variant ingestion converter. It does not close public integration.
29 existing paging/public-denial checks pass in66.74s; registered access proof
passes1 case in7.88s. No successful public graph retrieval is proven. Spec
consultation confirms paired citation supplies provenance target; ancestry and
policy list membership need the linked narrow design amendment. Native fact
feasibility passed1 test in53.60s; its portable log is in the linked design's
evidence directory. Three reviews require broader operation-arm/target-join
proofs. The user decision on retained policy context versus proven-applied
policies remains pending; the proposal is noncanonical.
No row has closed;15 baseline-complete and8 open remain.

## Verified Internal Preparation Checkpoint

The corrected internal access/conversion candidate passes5 projection tests
(18.18s) and21 access/workflow tests (290.48s), full Ruff, scoped Pyright and
identity hygiene. Portable commands, hashes and results are in
../observation-ledger/milestones/registered-ingestion-prep-evidence/manifest.json.
Bounded correctness inspection approves only this internal preparation.
Exact retained-record equality and canonical scope identity are enforced.
The authorization-time snapshot field was removed; atomic snapshot/time binding
remains required. All writers and local checks are finished. Public composition
is parked; provenance policy meaning still awaits the user's pending choice.
No requirement row closes:15 baseline-complete and8 open.

2026-09-08 owner decision: complete retained policy context is approved. The linked observed-provenance design operation is active for its corrected feasibility and review; the semantic-choice blocker is resolved.
