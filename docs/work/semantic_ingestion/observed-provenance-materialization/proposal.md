...
## Pending Design-Owner Decision

Choose complete retained policy context (recommended above) or applied-only
policy reporting. Applied-only requires an explicit persisted execution trace;
existing construction metadata cannot establish that meaning. The prior frozen
candidate and its findings are preserved under history/. The current proposal
corrects identity-policy and correction-path omissions; expanded target/all-arm
feasibility and final review remain after this semantic choice is settled.
