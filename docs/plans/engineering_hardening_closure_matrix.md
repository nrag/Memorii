...
| C10 | P1 | Orthogonal provider-call, extraction, fallback, and final-source outcomes distinguish success, partial output, abstention, provider/schema failure, and hybrid recovery; failed extraction never commits memory. | Rule/LLM/hybrid outcome tests and provider-ingestion failure tests. |
