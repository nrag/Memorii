"""Strict policy/evidence binding and canonical certificate recomputation.
...
class HeldBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            type(self.expected_authority) is not NumericAuthority
...
def _json(raw: bytes, limits: TransportLimits) -> Any:
    ...
    try:
        return json.loads(
            raw, object_pairs_hook=pairs, parse_int=integer, parse_float=no_number, parse_constant=no_number
        )
...
def _spec(value: Any, limits: TransportLimits) -> EncodingSpec:
    ...
    if (
        type(row["scale"]) is not int
        or not 1 <= row["scale"] <= limits.max_precision
...
def _gate(value: Any) -> Gate:
    ...
    if (
        type(row["method"]) is not str
        or row["method"] not in {"exact_binomial", "weighted_hoeffding"}
        or type(row["direction"]) is not str
        or row["direction"] not in {"upper", "lower"}
        or type(row["estimand"]) is not str
    ...
    method: Literal["exact_binomial", "weighted_hoeffding"] = (
        "exact_binomial" if row["method"] == "exact_binomial" else "weighted_hoeffding"
    )
    direction: Literal["upper", "lower"] = "upper" if row["direction"] == "upper" else "lower"
    estimand: Literal["cluster_any_failure", "cluster_macro_mean"] = (
        "cluster_any_failure" if row["estimand"] == "cluster_any_failure" else "cluster_macro_mean"
    )
    return Gate(
        ...
        direction,
        estimand,
        _quantity(row["threshold"]),
        _quantity(row["nominal_alpha"]),
        row["minimum_clusters"],
        _id(row["threshold_spec_id"]),
        _id(row["nominal_alpha_spec_id"]),
        _id(row["lower_spec_id"]),
        _id(row["upper_spec_id"]),
        _id(row["weight_spec_id"]),
        _id(row["event_value_spec_id"]),
        row["iid_declared"],
    )
...
def _policy(raw: bytes, limits: TransportLimits) -> Policy:
    ...
    for name, ceiling in (
        ...
        ("precision", limits.max_precision),
    ...
    result = Policy(
        ...
        row["precision"],
        row["output_cap"],
        _quantity(row["family_alpha"]),
        _id(row["family_alpha_spec_id"]),
        tuple(_spec(v, limits) for v in row["specs"]),
        tuple(_gate(v) for v in row["gates"]),
        tuple(_membership(v) for v in row["memberships"]),
    )
    if (
        len({s.encoding_spec_id for s in result.specs}) != len(result.specs)
...
def _validate_context(policy: Policy, binding: HeldBinding) -> None:
    ...
    if (
        context.authority != binding.expected_authority
    ...
    for expected in context.expected_gates:
        if (
            type(expected.iid_bernoulli_clusters_proven) is not bool
    ...
    for preverified in sorted(context.expected_gates, key=lambda g: locator_bytes(g.gate.locator)):
        ...
        if (
            len(
                {
                    q.unit(s)
                    for s in (gate.threshold_spec_id, gate.lower_spec_id, gate.upper_spec_id, gate.event_value_spec_id)
                }
            )
            != 1
        ):
            ...
            for event_id in member.expected_event_ids:
                ...
                if event.value is None:
                    value = (
                        ...
                        else (upper if gate.direction == "upper" else lower)
                    )
                else:
        ...
        if gate.method == "exact_binomial":
            ...
            if (
                not preverified.iid_bernoulli_clusters_proven
                or gate.estimand != "cluster_any_failure"
                or any(
                    meter.compare(o.weight, equal_weight) != 0 or o.lower != _ZERO or o.upper != _ONE
        ...
        derived.append(
            _Derived(gate, tuple(observations), threshold, alpha, interval, successes, mean, squared, exponent)
        )
...
def _build(policy: Policy, evidence: Evidence, binding: HeldBinding, meter: ArithmeticMeter) -> Certificate:
    ...
    decisions = holm_complete(
        claims, tuple(locator_bytes(g.gate.locator) for g in binding.context.expected_gates), family, meter
    )
    ...
    for item in derived:
        ...
        if item.gate.method == "exact_binomial":
            ...
            bound = invert_weighted_hoeffding(
                item.observations,
                item.threshold,
                item.gate.direction,
                decision.effective_alpha,
        ...
        if item.gate.direction == "upper":
            bound_outcome: Outcome = (
                "pass"
                if meter.compare(bound.upper, item.threshold) <= 0
                else ("fail" if meter.compare(bound.lower, item.threshold) > 0 else "inconclusive")
            )
        else:
            bound_outcome = (
                "pass"
                if meter.compare(bound.lower, item.threshold) >= 0
                else ("fail" if meter.compare(bound.upper, item.threshold) < 0 else "inconclusive")
            )
        combined: Outcome = (
            "pass"
            if decision.outcome == bound_outcome == "pass"
            else ("fail" if "fail" in (decision.outcome, bound_outcome) else "inconclusive")
        )
        results.append(
            GateResult(
                item.gate.locator,
                ...
                _number(item.weighted_mean),
                None if item.squared_sum is None else _number(item.squared_sum),
                _number(item.threshold),
                _number(item.alpha),
                _number(decision.effective_alpha),
                _computed(item.probability),
                _computed(bound),
                decision.rank,
                decision.outcome,
                bound_outcome,
    ...
    return Certificate(
        ...
        _number(family),
        all(r.outcome == "pass" for r in results),
        tuple(results),
    )
...
def _count_ctv(value: Any, meter: ArithmeticMeter) -> int:
    ...
    if type(value) in (tuple, list):
        ...
        return (
            len('{"$type":"' + tag + '","items":[')
            + sum(_count_ctv(v, meter) for v in value)
            + max(0, len(value) - 1)
            + 2
        )
    if type(value) is dict:
        return (
            len('{"$type":"map","entries":[')
            + sum(_count_string(k) + _count_ctv(v, meter) + 3 for k, v in value.items())
            + max(0, len(value) - 1)
            + 2
        )
    if is_dataclass(value) and not isinstance(value, type):
        ...
        return (
            len('{"$type":"map","entries":[')
            + sum(_count_string(field.name) + _count_ctv(getattr(value, field.name), meter) + 3 for field in declared)
            + max(0, len(declared) - 1)
            + 2
        )
...
def _evaluate(
    policy_stream: BinaryIO, evidence_stream: BinaryIO, binding: HeldBinding, limits: TransportLimits
    ...
    if (
        sha256(policy_bytes).hexdigest() != binding.policy_sha256
        or sha256(evidence_bytes).hexdigest() != binding.evidence_sha256
    ):
